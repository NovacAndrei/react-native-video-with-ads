...
- (void)requestAds:(NSString *) adTagUrl {
    if (!adTagUrl) {
        ...
        // Init player if tag url is not valid.
...
- (void)adsManager:(IMAAdsManager *)adsManager didReceiveAdEvent:(IMAAdEvent *)event {
    if (event.type == kIMAAdEvent_LOADED && self.onAdsLoaded) {
        ...
        // Init player after ad completed.
        _player = [AVPlayer playerWithPlayerItem:_playerItem];
        _contentPlayhead = [[IMAAVPlayerContentPlayhead alloc] initWithAVPlayer:_player];
        _player.actionAtItemEnd = AVPlayerActionAtItemEndNone;
...
- (void)setSrc:(NSDictionary *)source
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t) 0), dispatch_get_main_queue(), ^{
        ...
        [self playerItemForSource:source withCallback:^(AVPlayerItem * playerItem) {
            ...
            // Moved to requestAds, onAdError and onAdsCompleted because if the player is initialized before the ad has played a video still image is displayed!
            //_player = [AVPlayer playerWithPlayerItem:_playerItem];
            //_contentPlayhead = [[IMAAVPlayerContentPlayhead alloc] initWithAVPlayer:_player];
            //_player.actionAtItemEnd = AVPlayerActionAtItemEndNone;
